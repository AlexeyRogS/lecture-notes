...
\begin{document}
    ...
    \section{Continuity}
        ...
        \begin{theorem}[Sequential Definition of Continuity]\label{seq-cont}
        ...
        \begin{proof}\\
            \begin{enumerate}[\upshape(i)]
                ...
                $(x_n)_n$ is a bounded sequence (all the terms are contained within an interval), so by the Bolzano-Weierstrass theorem (\ref{BWT}), there exists a convergent subsequence $(x_{n_k})_k\in[a,b]$ with limit $x_0\in[a,b]$.\\
                Then since $f$ is continuous, $f(x_{n_k})\to f(x_0)$, hence $(f(x_{n_k}))_k$ is a bounded sequence by theorem \ref{cvg-seq-bounded}, but this is a contradiction since $\abs{f(x_{n_k})}>n_k\ \forall k$, so $f$ must be bounded.
                ...
                By the Bolzano-Weierstrass theorem (\ref{BWT}), there exists a convergent subequence $(x_{n_k})_k\in[a,b]$ with limit $x_0\in[a,b]$, and since $f$ is continuous, $f(x_{n_k})\to f(x_0)$.\\
        ...
        \begin{example}
            ...
            \textit{Proof}. Let $g$ be given by $g(x)=f(x)-x$. Note that by theorem \ref{cts-func-props}, $g$ is continuous on $[0,1]$. Also note that
            \begin{align*}
                g(0)&=f(0)-0=f(0)\geq0\\
                g(1)&=f(1)-1\leq1-1=0
            \end{align*}
            So $0\in[g(1),g(0)]$. If $g(0)=0$ or $g(1)=0$, then $0$ or $1$ are fixed points. Therefore, assume that $g(0)>0>g(1)$, and hence by the intermediate value theorem (\ref{ivt}), $\exists x_0\in(0,1)$ so that $g(x_0)=0$, i.e. $f(x_0)-x_0=0$, so $x_0$ is a fixed point.
        \end{example}
        \begin{corollary}
        ...
        \end{corollary}
        \begin{proof}
            Suppose there are two or more distinct points in $f(I)$, let $y\in(\inf f(I),\sup f(I))$. By the intermediate value theorem (\ref{ivt}), $\exists x\in I$ such that $f(x)=y$, hence $f(I)$ is an interval.
        \end{proof}
